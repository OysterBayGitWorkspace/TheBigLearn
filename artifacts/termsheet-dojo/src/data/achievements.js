...
export const ACHIEVEMENTS = [
  ...
  { id: "cat_master_lp", name: "Liquidation Lord", desc: "Master LP", bgColor: "#FFF0EB", borderColor: "#FF7B54", condition: s => ALL_QUESTIONS.filter(q => q.category==="Liquidation Preferences" && s.cardStates && s.cardStates[q.id] && s.cardStates[q.id].state >= 2).length >= 8 },
  { id: "cat_master_ad", name: "Dilution Shield", desc: "Master AD", bgColor: "#E8F5F1", borderColor: "#5BB5A2", condition: s => ALL_QUESTIONS.filter(q => q.category==="Anti-Dilution" && s.cardStates && s.cardStates[q.id] && s.cardStates[q.id].state >= 2).length >= 8 },
  { id: "cat_master_vsop", name: "VSOP Virtuoso", desc: "Master VSOP", bgColor: "#F0EBF5", borderColor: "#B8A0D2", condition: s => ALL_QUESTIONS.filter(q => q.category==="ESOP & VSOP" && s.cardStates && s.cardStates[q.id] && s.cardStates[q.id].state >= 2).length >= 8 },
  { id: "cat_master_exit", name: "Exit Artist", desc: "Master Exit", bgColor: "#FDECEE", borderColor: "#E8626C", condition: s => ALL_QUESTIONS.filter(q => q.category==="Exit & Transfer Rights" && s.cardStates && s.cardStates[q.id] && s.cardStates[q.id].state >= 2).length >= 8 },
